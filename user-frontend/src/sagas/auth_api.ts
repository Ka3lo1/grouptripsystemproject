...
function* post_register_request({ payload: { api_path, email, password, firstName, lastName = '', telephone = '' } }: PostRegisterAction): any {
  try {
    const data: AxiosResponse = yield call(api.post_api, { api_path, body: { email, password, firstName, lastName, telephone } });
    console.log(data);
